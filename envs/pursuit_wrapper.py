...
from envs.ma_gym.envs.pursuit import PursuitEnv
import gym
import torch
import numpy as np
...
class PursuitWrapper(PursuitEnv):
    ...
    def eval(self, policy, n_episodes=20, greedy=True, load_from_file=False, 
        ...
        for e in range(n_episodes):
            ...
            # Take precaution
            if type(info) == dict: 
                if 'prey_left' in info.keys():
                    prey_left += info["prey_left"]
        
        pbar.stop()
        policy.reset([True])
        prey_left_mean = prey_left / n_episodes
        avg_return = np.mean(episode_rewards)

        logger.log('EvalPreyLeftMean: {}'.format(prey_left_mean))
        logger.log('EvalAvgReturn: {}'.format(avg_return))
        if not load_from_file:
            tabular.record('EvalPreyLeftMeans', prey_left_mean)
