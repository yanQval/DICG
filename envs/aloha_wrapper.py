...
from envs.ma_gym.envs.aloha import AlohaEnv
import gym
import torch
import numpy as np
...
class AlohsWrapper(AlohaEnv):
    ...
    def eval(self, policy, n_episodes=20, greedy=True, load_from_file=False, 
        ...
        for e in range(n_episodes):
            ...
            # Take precaution
            if type(info) == dict: 
                if 'trans' in info.keys():
                    trans += info["trans"]
        
        pbar.stop()
        policy.reset([True])
        trans_mean = trans / n_episodes
        avg_return = np.mean(episode_rewards)

        logger.log('EvalTransMean: {}'.format(trans_mean))
        logger.log('EvalAvgReturn: {}'.format(avg_return))
        if not load_from_file:
            tabular.record('EvalTransMeans', trans_mean)
