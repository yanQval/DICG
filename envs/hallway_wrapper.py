...
from envs.ma_gym.envs.hallway import HallwayEnv
import gym
import torch
import numpy as np
...
class HallwayWrapper(HallwayEnv):
    ...
    def eval(self, policy, n_episodes=20, greedy=True, load_from_file=False, 
        ...
        pbar.stop()
        policy.reset([True])
        match_mean = win / n_episodes
        avg_return = np.mean(episode_rewards)

        logger.log('EvalWinMean: {}'.format(match_mean))
        logger.log('EvalAvgReturn: {}'.format(avg_return))
        if not load_from_file:
            tabular.record('EvalWinMeans', match_mean)
